...
double *setFlist(int *N)  //returns an ascending major scale
{
	...
	double majorScaleIntervals[] = {110} ;

	double K = 1200 / log10(2.0) ;
	double F, *flist, *f ;
	int i ;

	*N = 60 ; //number of tones in sequence
...
double *makeCanon(double *v1, double *v2, double *v3, double sequenceDur, double gapDuration, double *totalDur)
{
	...
	*totalDur = sequenceDur + 2 * gapDuration ;
	...
	stereoCopy(stereo, v2, sequenceDur, 0.1 ) ;
	// copy 3rd voice with delay of '2*gapDuration'
	stereoCopy(stereo, v3, sequenceDur, 0.2 ) ;
...
int main(void)
{
	double toneDur = 0.1 ;           // duration of each tone
	double silenceDur = 0 * toneDur ;   // duration of each silence
	...
	saveSoundfile("test.wav", canon, totalDur) ;
