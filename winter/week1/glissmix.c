...
//For mix sounds
void mixSound(double *output, double *input, int start, int samples)
{
	int i ;
	int j ;
...
//sound 1
	tone1 = SawtoothGliss(f2, f1, dB, dur, nPartials) ;
	envelope(tone, dur, T, A, N) ;
	playbuffer(tone, dur) ;
	samples = dur * SAMPLEING_RATE ;
	
	saveSoundfile("glissmix.wav", tone, dur) ;
	free (tone) ;

//sound 2
	tone2 = TriangleGliss(f1, f2, dB, dur, nPartials) ;
	start = 0 ;   //where it starts
	samples = dur * SAMPLING_RATE ;
	mixSound(tone, start, samples) ;
